...
@section('title', __('views.admin.purchase.payments.edit.title'))

@section('content')
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <form action="{{ route('admin.purchase.payments.update', [$item->no_pembayaran_beli]) }}" method="POST" class="form-horizontal form-label-left">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="text" class="form-control" value="{{ $item->no_pembayaran_beli }}" disabled>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <input id="tanggal_bayar" type="date" class="form-control" name="tanggal_bayar" value="{{ old('tanggal_bayar', optional($item->tanggal_bayar)->format('Y-m-d')) }}" required>
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <select id="no_invoice_beli" name="no_invoice_beli" class="form-control" required>
                            ...
                            @foreach($invoiceList as $invoice)
                                <option value="{{ $invoice->no_invoice_beli }}" {{ (string) old('no_invoice_beli', $item->no_invoice_beli) === (string) $invoice->no_invoice_beli ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                        <input id="jumlah_bayar" type="number" step="0.01" min="0" class="form-control" name="jumlah_bayar" value="{{ old('jumlah_bayar', $item->jumlah_bayar) }}" required>
                ...
                <div class="form-group">
                    <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                        <a class="btn btn-primary" href="{{ route('admin.purchase.payments.index') }}">{{ __('views.admin.purchase.payments.edit.cancel') }}</a>
                        <button type="submit" class="btn btn-success">{{ __('views.admin.purchase.payments.edit.save') }}</button>
                    </div>
                </div>
            </form>
        </div>
